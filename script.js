...
const senderRelation = document.querySelector("#sender-relation");
...
function showMessage(index) {
  ...
  nextButton.textContent = "次のメッセージ";
  updateProgress();
  showScreen("player");
...
// 動画終了または仮表示ボタンから、送信者照合を一度だけ開始します。
async function revealSenderInformation() {
  ...
  senderRelation.textContent = message.relation;
  senderStatus.textContent = message.status;
  senderCheck.hidden = true;
...
function getTransitionLines(nextIndex) {
  ...
  if (nextIndex >= 7) {
    return ["一致する知人：0名", "警告：照合結果が不安定です"];
  }
...
async function goToNextMessage() {
  ...
  getTransitionLines(nextIndex).forEach((line, lineIndex) => {
    const paragraph = document.createElement("p");
    paragraph.textContent = line;
    paragraph.classList.toggle("warning-copy", nextIndex >= 7 && lineIndex > 0);
    transitionCopy.append(paragraph);
  });

  await delay(nextIndex >= 7 ? 1850 : 1450);
  if (generation !== state.generation) {
    return;
  }
